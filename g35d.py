...
async def shutdown(application):
    ...
    try:
        ...
        logger.warning("Timeout durante la chiusura del bot: %s", e)
...
async def main():
    """Avvia il bot."""
    # Configura il timeout per le richieste HTTP
    request = HTTPXRequest(
        connect_timeout=5.0,  # Timeout di connessione (in secondi)
        read_timeout=5.0,     # Timeout di lettura (in secondi)
    )
